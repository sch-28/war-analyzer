<script lang="ts">
  ...
  */
	...
	function open_player(event: CustomEvent) {
		...
		if (last_selection)
			last_selection.childNodes.forEach((div: HTMLTableCellElement) => div.classList.remove("selected_player"));
		div.childNodes.forEach((div: HTMLTableCellElement) => div.classList.add("selected_player"));
		last_selection = div;
	...
	function update_graph() {
		...
		for (let i = 0; i < labels.length; i++) {
			if (i + 1 < labels.length) {
				...
				let logs = selected_guild.logs.filter((log) => log.time >= labels[i] && log.time < labels[i + 1]);

				// normalize all logs
				logs = logs.map((log) => log.normalized_log(selected_guild.name));

				// sort logs by time
				logs.sort((a, b) => (a.time < b.time ? -1 : 1));

				// calculate points
				let points = logs.reduce((sum, log) => sum + (log.kill ? 1 : -1), 0) / selected_guild.players.length;
				guild_data.push(points.toFixed(2));

				// a player is selected, do the same for it aswell
				if (selected_player) {
					logs = logs.filter((log) => log.player_one == selected_player.name);
					points = logs.reduce((sum, log) => sum + (log.kill ? 1 : -1), 0);
	...
	<div class="container_wrapper">
				...
				<div class="guild_select">
					{#each war.guilds as guild}
						<button
							...
							class:selected={guild == selected_guild}>{guild.name}</button
						>
					{/each}
				</div>
			</div>
		...
		<div class="container guild_detail">
			...
			<div class="view_stats_container">
				...
				<div class="buttons">
					<button class="button" on:click={() => navigate(`global/${selected_guild.name}`)}
						>{selected_guild.name}</button
					>
					<button
						class="button"
						on:click={() => navigate(`global/${selected_guild.name}/${selected_player.name}`)}
